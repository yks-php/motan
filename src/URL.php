...
class URL {
    ...
    private $_service;
    private $_method;
    private $_http_method = Constants::HTTP_METHOD_GET;     //GET
    private $_endpoint = Constants::ENDPOINT_AGENT;         //agent
    ...
    public function __construct($url)
    {
        if (!empty($url)) {
            ...
            $url_info = parse_url($url);                    //解析url，返回其组成部分
            $this->_path = ltrim($url_info['path'], '/');
            $this->_host = $url_info['host'];
            $this->_port = $url_info['port'];
            parse_str($url_info['query'], $this->_params);      //将query参数解析到 _params里面

            $this->_initMetaInfo(Constants::URL_SERVICE_KEY, '_service');       //从 _params里面将属性移出
            $this->_initMetaInfo(Constants::URL_GROUP_KEY, '_group');
            $this->_initMetaInfo(Constants::URL_SERIALIZE_KEY, '_serialize');   // ?? 并没有 _serialize 属性啊
